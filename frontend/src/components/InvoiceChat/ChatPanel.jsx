...
// ── Welcome message ───────────────────────────────────────
// Greeting එක backend එකෙන් එනවා — first message send කරද්දී
const WELCOME = {
  id:   "welcome",
  role: "bot",
  text: "Hi! Upload an invoice or ask me about your finances 💰",
};
...
export default function ChatPanel({ activeTab, onClose, userId }) {
  ...
  const [isFirstMessage, setIsFirstMessage] = useState(true); // greeting track
  ...
  const handleSendText = async () => {
    ...
    try {
      ...
      // Backend updated history save කරනවා
  ...
  const handleFileSelect = async (f) => {
    ...
    try {
      ...
      const res = await fetch(EXTRACT_URL(provider), {
        method: "POST",
  ...
  const handleSave = (data) => {
    addMessage({
      role: "bot",
      text: `✓ Saved — ${data.invoice_type === "income" ? "▲ Income" : "▼ Expense"} · ${data.currency || "LKR"} ${Number(data.amount).toLocaleString()}`,
    });
  };
    ...
    <div className="fixed bottom-24 right-6 z-50 w-80 h-[520px] bg-white rounded-2xl shadow-2xl border border-slate-200 flex flex-col overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-sm font-semibold text-slate-800 leading-tight">WealthTrack AI</p>
      ...
      <div className="flex flex-col gap-2 px-3 py-3 border-t border-slate-100 flex-shrink-0 bg-white">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-slate-400">
            Using <span className="font-semibold text-slate-600">{currentProvider.label}</span>
          </span>
        </div>
        <div className="flex items-center gap-2">
          {/* File upload */}
